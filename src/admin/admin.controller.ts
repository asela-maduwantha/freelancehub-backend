...
export class AdminController {
  ...
  @ApiOperation({
    summary: 'Perform action on user (suspend, activate, delete, make_admin)',
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  async performUserAction(
    ...
    const result = await this.adminService.performUserAction(
      userId,
      actionDto,
      req.user.sub,
    );
  ...
  private generateRecommendations(stats: any): string[] {
    ...
    if (growth.newUsers < 10) {
      recommendations.push(
        'Consider implementing user acquisition campaigns to increase new user registrations',
      );
    }

    if (growth.newProjects < 5) {
      recommendations.push(
        'Focus on encouraging existing users to post more projects through incentives',
      );
    }

    if (overview.activeUsers / overview.totalUsers < 0.3) {
      recommendations.push(
        'Implement user engagement strategies to increase platform activity',
      );
    }

    if (growth.revenue < 1000) {
      recommendations.push(
        'Review pricing strategy and consider value-added services to increase revenue',
      );
    }

    return recommendations.length > 0
      ? recommendations
      : ['Platform performance is strong across all metrics'];
  }
}
