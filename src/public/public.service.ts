...
export class PublicService {
  ...
  async getPlatformStats() {
    const [
      ...
      this.userModel
        .distinct('profile.location.country')
  ...
  async getCategories() {
    ...
    const categories = [
      ...
      },
      {
        ...
        description: 'Machine learning, data analysis, AI',
        icon: 'brain',
      },
      {
        id: 'video-animation',
        name: 'Video & Animation',
        description: 'Video editing, 3D animation, motion graphics',
        icon: 'video',
      },
      {
        id: 'business',
        name: 'Business',
        description: 'Consulting, project management, strategy',
        icon: 'briefcase',
      },
    ];
  ...
  async getFeaturedTestimonials(limit: number = 10) {
    const testimonials = (await this.reviewModel
      ...
      .populate(
        'reviewerId',
        'username profile.firstName profile.lastName profile.avatar roles',
      )
    ...
    return testimonials.map((testimonial: any) => ({
      ...
      userRole: testimonial.reviewerId.roles.includes('client')
        ? 'client'
        : 'freelancer',
  ...
  async getSkills({
      ...
      skillsData.map(async (skill) => {
        ...
        const avgRateResult = await this.userModel.aggregate([
          ...
          },
          {
            $group: {
              _id: null,
              avgRate: { $avg: '$freelancerProfile.hourlyRate.amount' },
            },
          },
        ]);
    ...
    if (popular) {
      filteredSkills = filteredSkills.filter(
        (skill) => skill.freelancerCount > 10,
      );
    }
  ...
  async getFeaturedFreelancers(limit: number = 8) {
    return this.userModel
      ...
      .select(
        'username profile freelancerProfile.title freelancerProfile.skills freelancerProfile.hourlyRate',
      )
      .sort({
